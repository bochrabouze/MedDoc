    ...
    <div class="card">
        <div class="card-body">
            <div class="d-flex justify-content-between pb-2 mb-2">
                <h5 class="card-title">Update Medical Visit Data</h5>
                <div>
            ...
            <form @submit.prevent="updateVisit" enctype="multipart/form-data">
               <div class="form-group mb-2">
                ...
                <button type="submit" class="btn btn-primary mt-4 mb-4"> Update Visit</button>
...
export default{
    ...
    created() {
        this.$axios.get('/sanctum/csrf-cookie').then(response => {
            this.$axios.get(`/api/visits/edit/${this.$route.params.id}`)
            .then(response => {
                this.date = response.data['date'];
                this.time = response.data['time'];
                this.diagnostic = response.data['diagnostic'];
                this.synthesis = response.data['synthesis'];
    ...
    methods: {
        updateVisit(e) {
            this.$axios.get('/sanctum/csrf-cookie').then(response => {
                ...
                this.$axios.post(`/api/visits/update/${this.$route.params.id}`, formData, config)
                .then(response => {
